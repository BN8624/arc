...
The pilot root `pilot_live_calls.json` is the only canonical live telemetry for the whole pilot, including acceptance calls. Each episode `live_calls.json` is a deterministic, rebuildable projection of only that episode scope: the same-scope calls in canonical root order plus only that scope's contract failures. Episode projections are operational files, not immutable hash-pinned artifacts; a legacy projection hash left in an episode manifest is ignored and removed on the next projection save.

A missing or stale-prefix episode projection is not corruption: status reports it as `RECONCILABLE` with `EPISODE_PROJECTION_STALE`, and both `pilot-live-reconcile` and a normal `pilot-live-run` resume rebuild it atomically from root telemetry before any provider call. Projection recovery never issues a provider call and never changes root telemetry, routing state, usage records, or canonical artifacts. Projection conflicts (calls absent from root, mutated or reordered calls, foreign or acceptance scopes, fabricated contract failures) and root telemetry corruption (duplicate call IDs or lease sequences, unknown scopes, changed checkpoint prefixes, routing state behind telemetry) fail closed without overwriting the conflicting file. This recovery contract repairs resume only; it does not demonstrate Phase 3 pilot acceptance or prose reliability, and synthetic rolling-plan transitions remain separate unresolved work.
