...
def acceptance_review_response(payload: dict, hold: bool = False) -> dict:
    ...
    if rule["require_first_and_last_episode"]:
        for target in (episode_ids[0], episode_ids[-1]):
            if not any(by_ref[ref]["episode_id"] == target and by_ref[ref]["kind"] != "transition" for ref in coverage):
                add(next(entry for entry in by_kind["episode_final"] if entry["episode_id"] == target))
    ...
    for criterion in payload["criteria"]:
        ...
        for kind in criterion["required_evidence_kinds"]:
            entries = [by_ref[ref] for ref in coverage if by_ref[ref]["kind"] == kind]
            if not entries:
                entries = [by_kind[kind][0]]
                add(entries[0])
            evidence.extend(entry["candidate_id"] for entry in entries)
        criterion_results.append({"criterion_id": criterion["criterion_id"], "result": "PASS", "finding": f"The cited artifacts keep {criterion['criterion_id']} consistent across the reviewed episodes.", "evidence_candidate_ids": evidence})
    critical = None
    if hold:
    ...
    strengths = [{"criterion_id": result["criterion_id"], "strength": f"Evidence for {result['criterion_id']} stays traceable to the selected catalog candidates.", "evidence_candidate_ids": list(result["evidence_candidate_ids"])} for result in pass_results[:1]]
    return {
        ...
        "proposal": {"dimension_result": "HOLD" if hold else "PASS", "criterion_results": criterion_results, "critical_finding": critical, "strengths": strengths},
    }
...
class MockModelClient:
    ...
    def _response(self, stage: str, role: str, prompt: str) -> dict:
        ...
        if stage == "pilot_review":
            return acceptance_review_response(payload, hold=payload.get("scenario") == "pilot_hold" and role == "continuity")
